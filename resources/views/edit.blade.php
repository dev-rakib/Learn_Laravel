...
<body>
    ...
    <main>
        <form action="{{ route('update', $mypost->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="name" id="" placeholder="enter Your Name" value="{{$mypost->name}}">
            @error('name')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
            <label for="desc">Description:</label>
            <input type="text" name="description" placeholder="Write Your Descripiton" value="{{$mypost->description}}">
            @error('description')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
